...
class BinaryTree(object):
  ...
  def insert(self, data):
    ...
    if self.root is None:		#If there is no node insert the node at the root
	self.root = insert_node
      ...
      current = self.root	
      while True:
        if data < current.data:   	#If the node is smaller than its root node insert at left
	...
	if data > current.data:
	  if current.right is None:    #If the node is greater than its root node insert at right
